...
Claude Desktop이나 교재에서 모르는 단어가 보일 때 `Command+F`(Mac) 또는 `Ctrl+F`(Windows)를 눌러 찾아보는 **화면 옆 참고사전**입니다.

## 가장 먼저 기억할 네 가지

1. Code의 모드는 기본값 **자동** 그대로 둡니다. 위험한 작업은 Claude가 먼저 물어봅니다.
2. 승인 창에서는 “어느 폴더의 어떤 파일을 바꾸는가”만 봅니다.
3. API key, 비밀번호, 카드번호는 채팅과 화면 인증에 올리지 않습니다.
4. 삭제와 결제 승인은 멈추고 운영자에게 묻습니다.
...
![홈의 Cowork, 폴더, 승인 방식, 모델과 Effort](day00-assets/screenshots/02_cowork_controls.png)

![Code의 로컬, 폴더 선택, 권한, 모델과 작업 강도](day00-assets/screenshots/03_code_controls.png)

수강 기본값은 `로컬(Local) / 오늘 폴더 / 화면의 기본 모델 / 높음(High)`이고, 모드는 기본값 자동 그대로입니다.

홈의 코워크와 Code는 기본값이 다릅니다. 코워크는 **수동 승인**(파일을 바꾸기 전에 물어봄)이 기본이고, Code는 **자동**(알아서 진행하되 위험한 작업 전에는 멈추고 물어봄)이 기본입니다. 둘 다 기본값 그대로 두면 됩니다.
...
| 자동 | Claude가 알아서 진행하되, 위험한 작업 전에는 멈추고 묻는 기본 모드 |
...
Day 0에서는 위 표만 이해하면 충분합니다. 아래 내용은 Day 1~7에서 낯선 말이 나타날 때 검색해 보는 참고용입니다.
...
- **이 과정에서 언제 봄:** Day 0의 첫 인사와 Day 1의 아티팩트 실습에서 봅니다.
...
- **이 과정에서 언제 봄:** Day 4에서 여행 메모 3개를 브리핑북과 PDF로 정리할 때 봅니다.
...
- **왕초보가 지금 할 행동:** `로컬 → 폴더 선택…` 순서로 수업 폴더를 엽니다. 모드는 자동 그대로 둡니다.

### 디자인(Design)

- **한 줄 뜻:** 대화창과 캔버스에서 디자인·대화형 시제품·발표 자료를 만들고, 필요하면 Claude Code로 넘길 수 있는 별도 베타 기능입니다.
- **생활 비유:** 말로 주문하면서 옆의 큰 도화지에 시안을 바로 그리는 디자인실입니다.
- **이 과정에서 언제 봄:** Day 3에서 커버 3안·카드뉴스 7장·스토리 1장을 만들 때 사용합니다.
- **왕초보가 지금 할 행동:** Day 3에서 사이드바의 **디자인**을 누릅니다. 조직 설정이나 요금제 때문에 메뉴가 없으면 임의 결제하지 말고 운영자에게 화면을 보여 줍니다.

### Artifact(아티팩트)

- **한 줄 뜻:** 채팅 대화 옆에서 따로 열리고 수정·다운로드·발행할 수 있는 문서, 코드, 시각화 또는 작은 앱 결과물입니다.
- **생활 비유:** 대화 중 나온 낙서가 아니라 따로 꺼내 쓰는 완성 작품판입니다.
- **이 과정에서 언제 봄:** Day 1에서 인스타 캡션·유튜브 제목과 썸네일 웹앱 2종을 만들 때 봅니다.
- **왕초보가 지금 할 행동:** 완성 작품에서 입력·생성·재생성·복사 버튼을 직접 눌러 봅니다. 왼쪽 `아티팩트` 메뉴에는 **게시한 작품만** 모입니다 — 방금 만든 것이 안 보이는 건 정상입니다. 게시한 뒤에는 그 목록에서 카드의 `⋮` → `링크 복사`로 주소를 다시 얻을 수 있습니다.
...
- **왕초보가 지금 할 행동:** 컴퓨터 앱의 `Cowork 왼쪽 메뉴 → 발송`에서 먼저 연결하고, 휴대폰 Claude 앱에서 `발송`을 엽니다. 승인 화면은 컴퓨터에서 직접 확인합니다.
...
- **한 줄 뜻:** 내 자료를 한 번 넣어 두면 그 안의 새 채팅에서도 다시 설명하지 않고 참고하게 하는 작업방입니다.
- **생활 비유:** 나를 소개하는 종이 한 장을 책상 위에 계속 펼쳐 둔 전용 상담실입니다.
- **이 과정에서 언제 봄:** Day 2에서 자료 4개와 지침을 넣고 LIFE BRAND 운영본부를 만들 때 봅니다.
- **왕초보가 지금 할 행동:** Day 2에서 `프로젝트 → 새 프로젝트 → 오른쪽 컨텍스트 + → 파일 추가` 순서로 제공 파일 네 개를 넣습니다.

### 프로젝트 폴더

- **한 줄 뜻:** Code에서 하나의 목표를 위해 관련 파일을 모아 둔 컴퓨터 폴더입니다.
- **생활 비유:** 작업에 필요한 설계도와 부품을 담은 상자입니다.
- **이 과정에서 언제 봄:** Day 5 이후 Code 실습에서 봅니다.
- **왕초보가 지금 할 행동:** Code에서는 오늘 과제 폴더 하나만 열고 다른 개인 폴더는 함께 열지 않습니다.
...
- **왕초보가 지금 할 행동:** 기본값 **자동** 그대로 둡니다. 고를 것이 없습니다.

현재 화면의 모드는 세 가지입니다.

| 화면 이름 | 무엇을 하는가 | 왕초보 행동 |
|---|---|---|
| **자동** | Claude가 권한 결정을 알아서 처리합니다. 위험하다고 판단한 작업은 실행하지 않고 먼저 물어봅니다. | **기본값입니다. 그대로 둡니다.** |
| **편집 자동 수락** | 모든 파일 편집을 묻지 않고 수락합니다. | 이 과정에서는 사용하지 않습니다. |
| **계획** | 파일을 바꾸기 전에 계획부터 만들어 보여 줍니다. | 5일차 큰 실습에서 설계를 먼저 볼 때 사용합니다. |

> 예전 교재나 영상에 나오는 **수동(Ask permissions)** 모드는 지금 화면에는 없습니다. 그 역할(위험한 작업 전에 묻기)은 **자동**이 이어받았습니다.
...
- **명령어(Command):** Terminal 안에 입력하는 정해진 작업 문장입니다. 교재의 한 줄 전체만 복사하고, 관리자 비밀번호·삭제·뜻 모르는 설치 명령이 나오면 실행하지 않습니다.
...
| 승인 질문을 건너뛰는 설정, 모든 승인 건너뛰기 | 사람이 파일 변경과 명령을 확인할 기회가 사라집니다. | 기본값(코워크 수동 승인, Code 자동)으로 되돌립니다. |
