...
1. 첫 실습은 권한을 **수동(Ask permissions)**으로 두고, 익숙해지면 자동을 써도 됩니다.
...
수강 기본값은 `로컬(Local) / 오늘 폴더 / 화면의 기본 모델 / 높음(High)`이고, 권한은 첫 실습만 수동입니다.
...
- **왕초보가 지금 할 행동:** 현재 화면에서는 **수동(Ask permissions)을** 고릅니다.
...
| **수동(Ask permissions)** | 파일 편집과 명령 실행 전에 묻고 Diff를 보여 줍니다. | **첫 7일 기본값입니다.** |
| **승인(Auto accept edits)** | 일반 파일 편집은 자동 수락하고, 그 밖의 터미널 명령은 계속 묻습니다. | 파일 변경 판단에 익숙해진 뒤 수업 지시가 있을 때만 사용합니다. |
| **자동(Auto)** | 배경 안전 검사를 거쳐 더 자율적으로 작업합니다. 연구 미리보기이며 특정 요금제·모델에서만 보일 수 있습니다. | 왕초보 기본 과정에서는 사용하지 않습니다. |

> 한국어 화면의 **수동·승인·계획·자동**은 공식 영문 문서의 **Ask permissions·Auto accept edits·Plan·Auto**와 각각 같은 모드입니다. 이 네 쌍만 같은 이름으로 기억하면 됩니다.
