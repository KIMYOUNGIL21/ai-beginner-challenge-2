...
1. 승인 창에서는 “어느 폴더의 어떤 파일을 바꾸는가”만 봅니다.
2. API key, 비밀번호, 카드번호는 채팅과 화면 인증에 올리지 않습니다.
3. 삭제와 결제 승인은 멈추고 운영자에게 묻습니다.
...
![Code의 로컬, 폴더 선택, 모델과 작업 강도](day00-assets/screenshots/03_code_controls.png)

수강 기본값은 `로컬(Local) / 오늘 폴더 / 화면의 기본 모델 / 높음(High)`입니다.
...
- **왕초보가 지금 할 행동:** `로컬 → 폴더 선택…` 순서로 수업 폴더를 엽니다.
...
- **왕초보가 지금 할 행동:** 채팅·카톡·스크린샷·GitHub에 절대 올리지 않습니다. 노출됐다면 카톡 글을 지우는 것으로는 부족합니다 — **그 키를 발급받은 서비스 사이트에서 키 자체를 삭제(폐기)하고 새 키를 발급**합니다.
...
| 승인 질문을 건너뛰는 설정, 모든 승인 건너뛰기 | 사람이 파일 변경과 명령을 확인할 기회가 사라집니다. | 그 설정을 켜지 않습니다. 이미 켰거나 모르겠으면 화면을 찍어 운영자에게 묻습니다. |
