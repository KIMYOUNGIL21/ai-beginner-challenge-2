...
1. Code에서는 권한을 **Manual(수동, 이전 명칭 Ask permissions)**로 둡니다.
...
- `홈`: Chat·Cowork·Projects·Artifacts·Design을 사용하는 대화 중심 공간
...
![Code의 Local, 폴더 선택, 권한, 모델과 Effort](day00-assets/screenshots/03_code_controls.png)

수강 기본값은 `Local / 오늘 폴더 / 화면의 기본 모델 / High(높음) / Manual(수동)`이다.

홈의 Cowork에서 보이는 **Manual**과 Code에서 보이는 **Manual**은 위치와 적용 대상이 다릅니다. Cowork Manual은 Cowork가 파일·앱을 사용하는 작업을 사람이 확인하는 방식이고, Code Manual은 파일 편집과 명령 실행을 승인하는 Code 권한 모드입니다. 둘 다 사람이 확인한다는 원칙은 같지만 같은 메뉴는 아닙니다.
...
- **왕초보가 지금 할 행동:** `Local → Open folder → Manual(수동)` 순서로 수업 폴더를 엽니다.

### Design(디자인)

- **한 줄 뜻:** 대화창과 캔버스에서 디자인·대화형 시제품·발표 자료를 만들고, 필요하면 Claude Code로 넘길 수 있는 별도 베타 기능입니다.
- **생활 비유:** 말로 주문하면서 옆의 큰 도화지에 시안을 바로 그리는 디자인실입니다.
- **이 과정에서 언제 봄:** 이번 기본 완주에는 필수가 아니며, 디자인 시안이나 Code 전달을 배우는 심화에서 봅니다.
- **왕초보가 지금 할 행동:** 사이드바에 보여도 지금은 누르지 않아도 됩니다. 조직 설정이나 요금제에 따라 보이지 않을 수도 있습니다.

### Artifact(아티팩트)

- **한 줄 뜻:** Chat 대화 옆에서 따로 열리고 수정·다운로드·발행할 수 있는 문서, 코드, 시각화 또는 작은 앱 결과물입니다.
...
- **한 줄 뜻:** 휴대폰에서 작업을 맡기고 같은 Cowork 또는 Code 세션을 컴퓨터에서 이어 보는 기능입니다.
- **생활 비유:** 외출 중 동료에게 일을 맡기고 사무실에서 결과를 이어 확인하는 것입니다.
- **이 과정에서 언제 봄:** 기본 과정 필수 기능이 아니며 장시간 작업의 심화에서 봅니다.
- **왕초보가 지금 할 행동:** 처음 7일은 컴퓨터 앞에서 승인 화면을 직접 보는 연습부터 합니다.
...
- **왕초보가 지금 할 행동:** **보이는 선택지 중 High를 수강 기본값**으로 둡니다. 단순 복사·분류·짧은 요약은 Medium, 여러 프로젝트 통합·복잡한 오류 해결은 High 이상을 씁니다.

> **Model과 Effort의 차이:** Model을 바꾸는 것은 담당자 자체를 바꾸는 것이고, Effort를 바꾸는 것은 같은 담당자가 생각에 쓰는 시간과 양을 조절하는 것입니다.

| 화면에 보이는 단계 | 쉬운 비교 | 잘 맞는 일 | 속도·사용량 |
|---|---|---|---|
| **Low** | 아주 빠르게 훑기 | 파일 이름 찾기, 한 줄 확인 | 가장 빠르고 사용량이 적지만 놓칠 수 있음 |
| **Medium** | 보통으로 살펴보기 | 단순 복사, 정리, 짧은 요약 | 빠르고 사용량을 아끼기 좋음 |
| **High** | 꼼꼼하게 검토하기 | 수업 과제, 일반적인 제작·수정·검수 | 품질·속도의 균형, **수강 기본값** |
| **Extra high / xhigh** | 더 오래 깊게 검토하기 | 긴 코딩, 여러 단계 에이전트 작업 | High보다 느리고 사용량이 많음 |
| **Max** | 가능한 가장 깊게 검토하기 | 정말 어려운 통합·복합 문제의 최종 점검 | 가장 느리고 사용량이 매우 큼; 과하게 생각할 수도 있음 |
공식 Claude Code 문서의 현재 단계는 모델에 따라 `Low`, `Medium`, `High`, `Extra high(xhigh)`, `Max`입니다. 일부 단계가 보이지 않을 수 있습니다. **없는 옵션을 찾지 말고, 내 화면에 보이는 선택지 중 High를 고르는 것이 이 과정의 기준**입니다.

> **Ultra 별도 참고:** 일부 앱·계정·신규 모델 화면에 `Ultra`처럼 별도 최고 선택지가 보일 수 있습니다. 공식 Claude Code의 위 단계표와 섞어 외우지 않습니다. 매우 느리고 사용량이 클 수 있으므로 이번 수업의 기본값이 아니며, 보여도 **High**를 선택합니다.

높은 Effort가 항상 더 좋은 것은 아닙니다. 높일수록 답이 더 꼼꼼해질 수 있지만 응답이 느려지고 token 사용량이 늘어 사용 한도에 더 빨리 닿을 수 있습니다.

위 실제 화면에서 오른쪽 아래의 모델 이름과 `높음`을 누르면 선택 메뉴가 열린다. 화면에 없는 단계를 찾지 말고 **High/높음**을 선택한다.
...
- **왕초보가 지금 할 행동:** 현재 화면에서는 **Manual(수동)**을 고릅니다. 공식 문서나 이전 화면의 **Ask permissions**가 같은 목적의 이전 명칭으로 보일 수 있습니다.

현재 Desktop 공식 문서에 안내된 주요 이름은 다음과 같습니다. 계정·요금제·조직 정책에 따라 일부가 보이지 않을 수 있습니다.

| 화면 이름 | 무엇을 하는가 | 왕초보 행동 |
|---|---|---|
| **Plan mode** | 파일을 읽고 조사해 계획을 제안하지만 소스 파일을 고치지 않습니다. | 큰 작업을 시작하기 전 계획만 보고 싶을 때 사용합니다. |
| **Manual(수동, 구 Ask permissions)** | 파일 편집과 명령 실행 전에 묻고 Diff를 보여 줍니다. | **첫 7일 기본값입니다.** |
| **Auto accept edits** | 일반 파일 편집은 자동 수락하고, 그 밖의 터미널 명령은 계속 묻습니다. | 파일 변경 판단에 익숙해진 뒤 수업 지시가 있을 때만 사용합니다. |
| **Auto** | 배경 안전 검사를 거쳐 더 자율적으로 작업합니다. 연구 미리보기이며 특정 요금제·모델에서만 보일 수 있습니다. | 왕초보 기본 과정에서는 사용하지 않습니다. |
| **Bypass permissions** | 승인 질문을 건너뛰고 실행합니다. | 격리된 테스트 환경 전문가용입니다. **선택하지 않습니다.** |
...
| `Bypass permissions`, 모든 작업 자동 승인 | 사람이 파일 변경과 명령을 확인할 기회가 사라집니다. | Manual(수동)로 되돌립니다. |
...
버튼 이름은 앱 언어와 버전에 따라 `Open folder / Select folder / 폴더 열기`, `Allow / Approve / 허용`, `Deny / Reject / 거절`처럼 달라질 수 있습니다. **뜻이 같아도 승인 전 대상 파일과 폴더를 확인하는 습관은 같습니다.**
...
- [Claude Artifacts 초보 튜토리얼](https://academy.claude.com/tutorials/use-artifacts-to-visualize-and-create-ai-apps-without-ever-writing-a-line-of-code)
