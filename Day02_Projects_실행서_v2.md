# Day 2 — Claude Projects로 나만의 AI 작업방 만들기

## 이 기능은 무엇인가요?

**Projects(프로젝트)**는 한 가지 일을 위한 자료와 규칙, 대화를 한곳에 모아 두는 Claude의 작업방입니다. 오늘 넣은 소개·과정·FAQ 문서는 그 프로젝트 안에서 새 대화를 열어도 다시 활용할 수 있습니다.

중요한 차이가 하나 있습니다. 프로젝트의 **이름과 설명만 적어서는 Claude가 그 내용을 아는 것이 아닙니다.** Claude가 계속 참고할 내용은 `Project knowledge(프로젝트 지식)`에 올리고, 계속 지킬 말투와 규칙은 `Project instructions(프로젝트 지침)`에 저장합니다.

2026년 8월 기준 Projects는 무료 계정에서도 사용할 수 있으며 무료 계정은 최대 5개 프로젝트를 만들 수 있습니다. 화면과 제공 범위는 바뀔 수 있습니다.
...
3. 왼쪽 메뉴의 **Projects** 또는 **프로젝트**를 한 번 누릅니다.
4. 프로젝트 목록 오른쪽 위의 **+ New Project** 또는 **새 프로젝트**를 누릅니다.
5. 이름을 적고 **Create**를 누르면 프로젝트 작업방이 열립니다.
6. 작업방 오른쪽 **Project knowledge**의 `+`는 참고 자료를 넣는 곳입니다.
7. **Set project instructions**는 Claude가 계속 지킬 말투와 규칙을 적는 곳입니다.
8. 가운데 **New chat**을 누르면 이 자료와 규칙을 사용하는 새 대화가 시작됩니다.

![Claude Desktop에서 Projects를 여는 실제 위치](day02-assets/screenshots/01_projects_location.png)

운영자 캡처 목록과 번호 콜아웃:

- 캡처 D2-1, 프로젝트 목록: `① 왼쪽 Projects` `② + New Project`
- 캡처 D2-2, 새 프로젝트 창: `① 프로젝트 이름` `② 설명` `③ Create`
- 캡처 D2-3, 프로젝트 안: `① New chat` `② Project knowledge의 +` `③ Set project instructions`
- 캡처 D2-4, 결과 화면: `① 생성된 결과` `② Publish` `③ Copy link`
...
|---|---:|---|---|
| 기능 이해 | 20분 | Projects 위치와 지식·지침 차이 확인 | 연습 프로젝트가 생김 |
| 작은 결과 3개 | 45분 | 세 자료를 올리고 세 결과 생성 | 채팅에 결과 3개가 보임 |
| 메인 결과물 | 45분 | 안내 페이지 Artifact 생성·게시 | 한 장 화면과 링크가 생김 |
...
1. **Projects → + New Project**를 누릅니다.
2. 이름을 `왕초보2기 Day2 닉네임`으로 적습니다.
3. 설명은 `AI 첫걸음 과정 안내물을 만드는 연습방`이라고 적습니다.
4. **Create**를 누릅니다.
5. **Set project instructions**를 열어 아래 문장을 그대로 붙여넣고 저장합니다.

```text
나는 컴퓨터 왕초보입니다.
항상 쉬운 한국어와 짧은 문장으로 답해 주세요.
프로젝트 지식에 있는 사실만 사용하고, 없는 날짜·가격·성과는 만들지 마세요.
...
1. 오른쪽 **Project knowledge**의 `+`를 누릅니다.
...
프로젝트 안에서 **New chat**을 누르고 아래 프롬프트를 차례대로 보냅니다. 한꺼번에 보내지 않습니다.

### 결과 1: 핵심 사실 카드

```text
프로젝트 지식의 과정정보만 확인해서 아래 네 칸을 채워 주세요.
...
```text
프로젝트 지식의 자주 묻는 질문에서 초보자에게 가장 중요한 질문 5개를 골라 주세요.
...
요구사항:
- 휴대폰에서 읽는 세로형 HTML Artifact로 만드세요.
...
버튼이 움직이고 글자가 휴대폰 크기에서 읽히면 **Publish(게시)**를 누르고 공개 링크를 복사합니다. Free·Pro·Max는 공개 게시가 가능하지만, Team·Enterprise는 조직 내부 공유만 가능할 수 있습니다. Publish가 없다면 결과 화면 캡처와 내려받은 파일로 기본 완주합니다.
...
| 보이는 상황 | 할 일 |
|---|---|
| Projects가 안 보임 | 앱을 최신 버전으로 업데이트하고 [claude.ai/projects](https://claude.ai/projects)를 브라우저에서 엽니다. |
| 파일을 여러 개 못 고름 | 한 파일씩 `+`를 세 번 눌러 올립니다. |
| Claude가 엉뚱한 숫자를 씀 | `프로젝트 지식에 없는 숫자는 삭제하고 출처 파일명을 옆에 써 주세요.`라고 보냅니다. |
| 결과 화면이 안 생김 | `설명문이 아니라 실제로 누를 수 있는 HTML Artifact를 만들어 주세요.`라고 보냅니다. |
| Publish가 없음 | 게시를 멈추고 결과 전체 화면 캡처와 내려받기 버튼이 있으면 HTML 파일을 저장합니다. |
...
```text
[왕초보2기 Day 2 / 닉네임]
오늘 만든 것: 자료와 규칙을 기억하는 Claude Project + 한 장 안내 페이지
내 주제: ___
결과물: 공개 링크 또는 대표 화면 첨부
Projects에서 배운 것: 새 대화에서도 프로젝트 지식과 지침을 다시 쓰는 방법
...
- [Anthropic 공식 — Artifact 게시와 공유](https://support.claude.com/en/articles/9547008-publish-and-share-artifacts): 요금제별 Publish와 Share 차이, 첨부파일 공유 주의
