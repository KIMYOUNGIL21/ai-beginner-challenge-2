...
4. 어제 이름을 바꿨던 Day 6 폴더(예: `Day06_Skill_닉네임`)를 여세요. 그 **안에 있는** `day06-handoff` 폴더를 꺼내 `day07-start` 안으로 옮기세요. `day07-start` 안에도 안내문만 든 같은 이름 폴더가 미리 들어 있어서, 옮기는 방식에 따라 **"대체" 또는 "병합"을 물을 수도, 창 없이 그냥 합쳐질 수도** 있습니다 — 어느 쪽이든 그대로 진행해도 안전합니다. 옮긴 뒤에는 `day07-start/day06-handoff` 폴더를 열어 `brief.json`과 `shorts-brief.html` 두 파일이 보이는지로 확인하세요.
