...
> 기본 예시는 `한옥 처마가 여름빛은 막고 겨울빛은 들이는 이유`입니다. 사람을 주인공으로 바꾸고 싶다면 `육아 중인 주부의 저녁 20분 주방 정리` 또는 `은퇴를 앞둔 50대의 경력 기록 정리`를 고릅니다.
...
| **수동(Ask permissions)** | 채팅 입력칸 아래 모드 단추 | 설치·외부 호출 전에 내가 확인 |
...
4. 입력칸 아래 모드를 **수동(Ask permissions)**으로 둡니다.
...
4. 휴대폰에서도 같은 Claude 계정으로 로그인하고 **Cowork → 발송**을 엽니다.
...
```text
[왕초보 챌린지 2기 Day 7 / 닉네임]
완성 주제: __________
내가 직접 한 일: 대본 승인 / 목소리 선택 / Flow 5컷 확인 / 휴대폰 재생
Claude가 한 일: 길이 계산 / 다운로드 배치 / 세로 영상·음성·자막 조립
결과물: D07_[닉네임]_쇼츠.mp4
검수: 1080×1920 / 음성 있음 / 자막 있음 / 재생 확인
다음 편: D07_[닉네임]_다음편.md
오늘 가장 신기했던 것: __________
```

MP4와 인증문을 함께 올립니다. API 키나 크레딧 화면은 올리지 않습니다.
