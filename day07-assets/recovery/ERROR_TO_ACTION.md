...
| Python 3가 없음 | 조립 스크립트를 실행할 도구가 없음 | OS와 화면을 운영자에게 보냄 |
| `ffmpeg not found` | 영상을 자르고 붙일 도구가 없음 | 임의 설치를 멈추고 운영자 안내 요청 |
| Pillow 오류 | 자막 이미지를 그릴 Python 환경이 다름 | 설치된 Skill의 `.venv` 사용 여부 확인 |
| Typecast 연결 정보 없음 | 로컬 비밀 설정이 없음 | `TYPECAST_SETUP.md`를 다시 실행 |
| Typecast 401/403 | 키 또는 API 접근 권한 문제 | 공식 계정 화면에서 키 상태 확인, 노출 없이 재등록 |
| Typecast 402/크레딧 부족 | 현재 API 크레딧 사용 불가 | 결제하지 말고 운영자에게 기본 경로 요청 |
...
같은 오류가 두 번 반복되면 추가 설치·삭제를 멈추고 운영자에게 `Day 7 / 단계 / OS / 오류 화면`을 보냅니다.
