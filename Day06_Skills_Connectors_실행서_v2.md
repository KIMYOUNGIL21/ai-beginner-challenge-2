...
오늘은 **Skill**을 써요. Skill은 미리 적어 둔 작업 순서표예요. `/shorts-brief` 하나만 부르면 Claude가 매번 같은 형식으로 `훅 3안 + 5컷 숏츠 주문서`를 만들어 줘요. 오늘 만든 파일 두 개는 Day 7에 실제 영상을 만들 때 그대로 써요.

오늘 할 일은 **다섯 단계**예요. 약 1시간이면 돼요. 설치할 것도, 외울 것도 없어요.
...
2. 압축을 풀면 `day06-assets` 폴더가 생겨요. **새 폴더를 만들지 말고, 이 폴더의 이름만** `Day06_Skill_닉네임`으로 바꾸세요.
3. 이름을 바꾼 폴더를 열어 보세요. `shorts-brief-maker.html` 파일이 **바로** 보이면 준비 끝이에요. 그 파일이 또 다른 폴더 안에 들어 있다면, 그 안쪽 폴더를 꺼내 바탕화면으로 옮기고 그 폴더 이름을 `Day06_Skill_닉네임`으로 바꾸세요.
4. Claude Desktop에서 **Code → 로컬 → 폴더 선택…**을 누르고, 방금 이름을 바꾼 바로 그 `Day06_Skill_닉네임` 폴더를 여세요.
...
> 💡 **내 주제로 바꾸면 훨씬 재밌어요!**
> 각 줄의 **`:` 뒤 내용만** 내 것으로 바꿔 보세요. 아이 간식, 베란다 화분, 매일 걷는 산책길… 내 일상에서 하나 고르면 Day 7에 만들 영상도 **내 이야기**가 돼요. 예시 그대로 보내도 돼요.

Claude가 훅 3안과 5컷 계획을 먼저 보여 줘요. 읽어 보고 마음에 들면 아래를 보내세요.
...
2. **제목, 훅 3개, C1부터 C5까지 컷 다섯 개**가 보이는지 확인하세요.
...
1. 오늘 폴더(`Day06_Skill_닉네임`) 안에 `day06-handoff`라는 새 폴더를 만드세요.
2. `input` 폴더의 `brief.json`을 복사해서 `day06-handoff` 안에 넣으세요.
3. `output` 폴더의 `shorts-brief.html`도 복사해서 같은 `day06-handoff` 안에 넣으세요.

Day 7 시작 패키지를 풀면, 이 `day06-handoff` 폴더를 `day07-start` 폴더 안으로 그대로 옮겨요. 옮길 때 창이 뜨면 `대체`나 `병합` 어느 쪽을 눌러도 돼요. 원래 들어 있던 건 안내문 한 장뿐이라서요.

> Day 7은 `brief.json`을 읽어 대본을 자동으로 만들어요. 오늘 쓴 문장을 다시 타이핑할 일은 없어요.
...
| 보이는 상황 | 바로 할 일 |
|---|---|
| "shorts-brief Skill이 없습니다"라는 답 | 지금 연 폴더 안에 `shorts-brief-maker.html`이 **바로** 보이는지 Finder/탐색기에서 확인하세요. 또 다른 폴더 속에 있으면 1단계 3번대로 꺼낸 뒤, Code에서 그 폴더를 다시 여세요 |
