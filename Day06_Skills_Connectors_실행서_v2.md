...
5. **연습 JSON 받기**와 **완성 HTML 받기**를 각각 한 번 눌러 두 파일이 내려오는지 확인합니다.

이 단계는 Skill을 쓰기 전에 최종 결과와 전달 파일을 손으로 확인하는 작은 성공입니다. 방금 **다운로드 폴더**에 받아진 연습용 `brief.json`·`shorts-brief.html` 두 개는 지워도 됩니다. 다음 단계부터 Skill이 **프로젝트 폴더 안에** 같은 이름의 진짜 파일을 만드는데, **그쪽은 지우면 안 됩니다.**
...
아래를 그대로 보냅니다. 첫 결과와 눈에 띄게 달라지는지 보는 것이 목적입니다.
...
내 주제로 하고 싶다면 각 줄의 **`:` 뒤 내용만** 내 것으로 바꿉니다. 줄 이름과 `/shorts-brief`는 그대로 둡니다.
...
5. Day 7 시작 패키지를 풀면 이 `day06-handoff` 폴더를 `day07-start` 폴더 안으로 그대로 옮깁니다. `day07-start` 안에 같은 이름의 폴더가 이미 있어 **"대체하시겠습니까"라고 물으면 대체를 눌러도 안전합니다** — 안에는 안내문 한 장뿐입니다.
