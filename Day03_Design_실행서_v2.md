...
| 인라인 코멘트 | 캔버스에서 한 요소만 고치기 | 제목·색·사진 중 하나 변경 |
...
> 위 그림은 **실제 화면 캡처가 아닌 위치 설명도**입니다. ① 왼쪽 채팅에서 처음 만들기를 부탁하고, ② 오른쪽 캔버스에서 결과를 보고, ③ 고칠 요소는 **캔버스에서 클릭해 그 자리에 코멘트로** 요청하고, ④ 계정 화면에 보이는 내보내기로 저장하는 순서만 기억합니다.
...
**중요 — 이 세 번은 왼쪽 채팅에 쓰지 않습니다.** 캔버스에서 고칠 요소를 클릭하면 그 자리에 코멘트를 달 수 있습니다. 거기에 아래 문장을 씁니다. 왼쪽 채팅에 쓰면 디자인 전체가 바뀔 수 있습니다.
...
### 7-1. 새 프로젝트를 열고 자료를 넣습니다

1. **디자인 → 새 프로젝트**를 누릅니다. 연습 결과와 섞이지 않게 새 방에서 시작합니다.
2. `brand-style.txt`와 내 사용 가능 사진을 첨부합니다. 사진이 없으면 첨부하지 않아도 됩니다.
3. Day 2에서 `D02_닉네임_Day3브리프.txt`를 저장했다면 그 파일도 함께 첨부합니다.
...
1. 오른쪽 위 **내보내기**를 누릅니다.
2. 보이는 형식 중 **PDF**를 고릅니다. 파일은 보통 `다운로드` 폴더에 저장됩니다.
3. 내보내기가 안 보이거나 저장이 되지 않으면 **캡처로 대신합니다.** 커버와 7장 전체가 보이게 화면을 캡처하면 됩니다.

교재에 없는 PNG 버튼을 찾지 마세요. 계정에 실제로 보이는 형식이면 무엇이든 괜찮고, **캡처만으로도 오늘은 완주입니다.**
...
| 한 곳 수정했는데 전체가 바뀜 | 왼쪽 채팅에 `직전 상태로 되돌려 주세요`를 보낸 뒤, 캔버스에서 요소를 클릭해 그 자리에 코멘트로 다시 요청합니다. |
| 내보내기 형식이 교재와 다름 | 없는 메뉴를 찾지 말고, 계정에 보이는 형식이나 화면 캡처를 씁니다. 캡처로도 인증됩니다. |
