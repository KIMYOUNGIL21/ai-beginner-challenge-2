...
![Claude Desktop에서 Cowork, 폴더, 수동, 모델과 Effort를 찾는 실제 화면](day04-assets/screenshots/01_cowork_mode.png)

1. 입력창 안쪽의 **Cowork**를 누릅니다.
2. 아래쪽 **프로젝트 또는 폴더**를 누릅니다.
3. 손 모양 옆 작업 방식은 **수동**으로 둡니다.
4. 오른쪽 모델은 수업에서 안내한 모델을 고릅니다.
5. **Effort는 높음**으로 둡니다. 높음이 보이지 않으면 기본값 그대로 진행합니다.

> 화면 이름은 앱 버전에 따라 조금 달라질 수 있습니다. 위 실제 화면처럼 `Cowork`, `프로젝트 또는 폴더`, `수동`을 찾으면 됩니다. **발송**은 다른 날 쓰는 기능이며, 오늘은 누르지 않습니다.
...
1. 휴대폰에서 연 브리핑북 첫 화면 캡처 1장
2. 공개 가능한 PDF
3. 아래 인증문

```text
[왕초보2기 Day 4 / 닉네임]
오늘 만든 것: 흩어진 메모 3개로 만든 가족 여행 브리핑북
Cowork에 맡긴 범위: Day04 연습 폴더 1개
내가 직접 확인한 것: 날짜 / 예산 / 일정 / 버튼 작동
가장 마음에 든 부분: ___
다음에 정리해 보고 싶은 자료: ___
```
