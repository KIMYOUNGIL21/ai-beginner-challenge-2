...
Claude Desktop을 열면 대화 입력칸 주변에 작업 방식을 고르는 **모드 선택기**가 있습니다. 여기에서 `Cowork`를 누릅니다. 버전에 따라 홈 화면의 Cowork 단추 또는 입력창의 모드 메뉴로 보일 수 있습니다.
...
4. 그 오른쪽 작업 방식은 **Manual(수동)**로 둡니다.
5. 오른쪽의 모델 메뉴에서 Effort를 **High(높음)**로 둡니다.

Cowork의 **Manual(수동)**은 Cowork가 파일이나 앱을 사용하는 작업을 사람이 확인하는 방식입니다. 이것은 다음 날 Code의 파일 편집·명령 권한 모드와 위치가 다릅니다. 오늘은 둘 다 자동 실행보다 사람이 확인하는 설정을 사용합니다.

Effort의 공식 단계는 모델에 따라 Low, Medium, High, Extra high(xhigh), Max입니다. 이번 수업 기본값은 **보이는 선택지 중 High**입니다. 단순 요약은 Medium도 충분하지만 오늘처럼 여러 파일을 묶는 작업은 High를 사용합니다. `Ultra`가 별도로 보여도 이번 기본값은 아닙니다.

## 오늘 쌓이는 결과물

![Day 4 메모 세 개를 합친 한 장 행사 안내문의 실제 완성 예시](day04-assets/expected/event-guide-example.png)

[완성 예시를 큰 화면으로 열기](day04-assets/완성예시_행사안내.html)
...
폴더 안에 `01_행사메모.txt`, `02_진행내용.txt`, `03_주의사항.txt`가 있으면 성공입니다. `완성예시_행사안내.md`는 막혔을 때 비교하는 답안이므로 먼저 복사하지 않습니다.

## 2. Cowork를 열고 안전 범위를 정합니다

1. Claude Desktop을 엽니다.
2. 입력창 주변의 모드 선택에서 **Cowork**를 고릅니다.
3. 처음 안내 화면이 나오면 내용을 읽고 **계속** 또는 **시작**을 누릅니다. 별도 명령어는 입력하지 않습니다.
4. 폴더 추가 또는 파일 추가 단추를 눌러 `Day04_Cowork_닉네임`을 선택합니다.
5. 접근 확인 화면에서 폴더 이름이 맞는지 봅니다.

버튼 이름은 버전에 따라 `Add folder`, `폴더 추가`, `Attach`처럼 조금 다를 수 있습니다. 폴더 전체 경로가 내 연습 폴더로 끝나는지만 확인합니다.
...
```text
01_행사메모.txt, 02_진행내용.txt, 03_주의사항.txt의 사실만 사용해 행사 안내문을 만들어 주세요.
...
오늘 폴더 안에 `내메모.txt`를 만듭니다. 메모장 또는 텍스트 편집기에 아래 다섯 줄만 적습니다.
