...
2. 왼쪽 아래 프로필 또는 설정을 연다.
3. **Settings → Capabilities**로 이동한다.
4. **Artifacts** 또는 한국어 화면의 **시각 자료**를 켠다.
5. **Code execution and file creation**도 켠다.

메뉴 이름이 조금 다르면 Claude에게 화면을 캡처해 보내지 말고, 강사에게 손을 든다. 계정 이메일이나 결제 화면은 오픈카톡에 올리지 않는다.
