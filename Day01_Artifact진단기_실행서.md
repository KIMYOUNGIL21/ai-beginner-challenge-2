...
| 작은 성공 만들기 | 5~10분 | 입력과 버튼이 작동 |
| 제조기 만들기 | 35분 | 입력 3칸·복사 버튼 |
| 내 것으로 바꾸기 | 25분 | 내 제목·예시·색상 |
| 실제 답 받기·저장·인증 | 35분 | Claude 답변과 PNG |

첫 로그인이나 게시 기능 확인은 20분 정도 더 걸릴 수 있습니다. 게시가 끝내 보이지 않으면 시간을 더 쓰거나 결제하지 않고 PNG 두 장으로 완주합니다.

---

## 1 — 완성 예시로 먼저 성공하기

위에서는 오늘의 전체 흐름을 30초 안에 훑어봤습니다. 이제 같은 흐름을 직접 눌러 실제 답변까지 받아 봅니다.
...
5. **결과 복사**를 누릅니다.
6. Claude Desktop의 **새로 생성 → 채팅**을 누릅니다.
7. 복사한 내용을 붙여넣고 보냅니다.
...
이것은 메인 제조기 전에 **입력과 버튼이 실제로 생기는 경험만 하는 5~10분 연습**입니다. 저장하거나 꾸미지 않고, 아래 성공 화면을 확인하면 바로 3단계의 메인 제조기로 이동합니다.
...
> 공개 인증에 쓸 실습에는 고객 이름·전화번호·이메일·회사 내부 정보·미공개 자료를 넣지 않습니다. 실제 업무가 민감하다면 같은 종류의 **가상 예시**로 바꿔 실습합니다.

1. 세 칸에 내 실제 일을 입력합니다.
2. **요청문 만들기 → 결과 복사**를 누릅니다.
3. Claude Desktop에서 **새로 생성 → 채팅**을 누릅니다.
4. 붙여넣고 보냅니다.
5. 실제 답변을 끝까지 읽습니다.
...
새 채팅에서 받은 실제 답변도 첫 부분이 보이게 캡처합니다. 파일명은 `D01_닉네임_실제답변.png`입니다. 캡처 전 입력문과 답변 안에도 고객 이름·전화번호·이메일·회사 내부 정보·미공개 자료가 없는지 확인합니다. 다른 대화 목록과 결제 화면도 넣지 않습니다.
...
게시가 없으면 앱을 업데이트한 뒤 [claude.ai](https://claude.ai/)의 같은 대화에서 확인합니다. 게시 확인에는 첫 로그인 때문에 20분 정도 더 걸릴 수 있습니다. 그래도 없으면 시간을 더 쓰거나 결제하지 않고 PNG 두 장만 제출하면 완주입니다.

## 8 — 오픈카톡 인증

올리기 직전에 PNG 두 장과 아래 문구를 다시 읽습니다. 고객 이름·전화번호·이메일·회사 내부 정보·미공개 자료가 하나라도 보이면 올리지 않고, 같은 종류의 가상 예시로 다시 캡처합니다.
