...
|---|---|---|
| 1. Creator Pulse | 내 콘텐츠 성과 대시보드 | 기간 변경, 차트, TOP 3, 다음 행동 |
| 2. Caption Atelier | 인스타 콘텐츠 스튜디오 | 정보 입력, 캡션 3종 생성, 복사 |
...
ZIP을 받은 뒤 압축을 풀고 `day01-assets → input → creator-performance.csv`를 찾습니다. 데이터만 받는 링크에서 글자 표가 보이면 `Command + S` 또는 `Ctrl + S`로 저장해도 됩니다. 파일 이름은 `creator-performance.csv` 그대로 둡니다. 작품 2는 사진 없이 진행합니다. 게시물 미리보기는 Claude가 만든 색상 표지로 나옵니다.
...
- **파일 올리기 칸은 만들지 마세요.** 아티팩트 안에서는 내 컴퓨터 파일을 못 불러옵니다
