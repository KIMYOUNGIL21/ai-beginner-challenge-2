...
| 흐름 | 배우는 기능 | 눈에 보이는 성공 |
|---|---|---|
| 기능 공부 | Code, 폴더, `계획` 모드, `자동` 모드, Diff, Preview | 계획과 실행을 나눠 확인한다 |
...
> **오늘은 반드시 Claude Desktop 앱에서 합니다.** 인터넷 브라우저(claude.ai)의 Code에는 내 컴퓨터 폴더를 여는 기능이 없습니다.

| 이름 | 쉬운 뜻 | 오늘 하는 일 |
|---|---|---|
| Code | 폴더의 파일을 만들고 고치는 작업 공간 | HTML 웹 도구 만들기 |
| 로컬 | 내 컴퓨터 안에서 작업 | 오늘 폴더 선택 |
| `자동` | **기본 모드.** Claude가 권한 결정을 알아서 처리하고, 위험한 작업은 스스로 막고 물어봅니다 | 그대로 두기 |
| `계획` | 변경하기 전에 계획부터 만드는 모드 | 큰 실습의 설계 먼저 확인 |
| Diff | 바꾸기 전과 후를 비교하는 화면 | 예상 파일만 바뀌는지 확인 |
| Preview/브라우저 | 완성 화면을 직접 누르는 곳 | 버튼 네 가지 검수 |

입력창 왼쪽 아래의 모드 단추를 누르면 아래 세 가지가 나옵니다. **오늘 쓰는 것은 `자동`과 `계획` 둘뿐**입니다.

![Code의 모드 메뉴 — 자동이 기본값입니다](day05-assets/screenshots/02_code_mode_menu.png)

> 예전에 있던 **`수동` 모드는 없어졌습니다.** 지금은 **`자동`이 기본값**이고, Claude가 위험하다고 판단한 작업은 실행하지 않고 물어봅니다. 찾지 마시고 그대로 두세요.

오른쪽의 **모델과 노력(Effort)도 손대지 마세요.** 기본값 **높음**이 오늘 필요한 값입니다.

![노력 메뉴 — 높음이 기본값입니다](day05-assets/screenshots/03_effort_menu.png)

> 노력을 `엑스트라`나 `최대`로 올리면 답이 느려지고 **하루 사용량을 훨씬 빨리 씁니다.** 7일을 완주해야 하니 기본값 그대로 두세요.
...
5. 입력창 왼쪽 아래 모드가 **`자동`** 인지만 확인합니다. 기본값이라 대부분 그대로입니다.
...
먼저 입력창 왼쪽 아래의 모드 단추를 눌러 **`계획`** 으로 바꿉니다. 계획 모드에서는 아직 파일을 만들지 않고, 무엇을 만들지 먼저 확인합니다. 아래 프롬프트는 줄이지 말고 한 번에 붙여넣습니다.
...
- 내가 계획을 승인하고 `자동` 모드로 되돌린 뒤 만들고, Preview에서 빈 입력, 새 작전, 완료 3회, 휴대폰 폭을 직접 점검합니다.
- 점검 결과를 통과/수정 필요 표로 알려 주세요.
```

계획에 `index.html 하나 생성`, 설치 없음, 기존 파일 보존이 보이면 성공입니다. 이제 모드 단추를 다시 눌러 **`자동`** 으로 되돌린 뒤 보냅니다.
...
Diff에 `index.html` 하나만 보이는지 확인합니다. 승인 창이 뜨면 파일 이름을 보고 허용합니다. 이 동선으로 **`계획` → 계획 확인 → `자동` → 파일 확인 → 실행**을 실제로 한 번씩 사용했습니다.
...
아래를 그대로 보냅니다. 한 번에 도구 전체가 다른 사람의 것으로 바뀝니다.
...
내 상황으로 하고 싶다면 위 문장에서 **작은따옴표 안의 이름과 할 일, 말투, 색만** 내 것으로 바꿔 보냅니다. 나머지 줄은 그대로 둡니다.
